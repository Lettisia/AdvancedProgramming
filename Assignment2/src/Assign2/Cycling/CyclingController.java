...
/*
 * Controller class of cycling events scene
 * 
 * Author: Théo DUFORT
 */
public class CyclingController implements Initializable {
	...
	ObservableList<String> athleteIDlist = FXCollections
			.observableArrayList(Main.getMyDriver().getAthletesNames(Main.getMyDriver().getCyclists()));

	/*
	 * Initializes AthletesView list
	 */
	@Override
	...
	public void buttonAction(ActionEvent event) throws IOException, GameFullException {
		...
		if (names.size() < Game.getMinCompetitors())
			...
			Main.getMyDriver().selectAthwithInt(Main.getMyDriver().getCyclists(), nums);
			Main.showOfficialScene();
		}
	}

	/*
	 * Handles press ENTER key same way as click button above
	 */
